  ...
  return (
    <header className="header-main">
      <div className="header-container">
        <div className="logo-section" onClick={() => navigate("/app/dashboard")}>
                ...
                <div className="absolute right-0 mt-3 w-72 bg-white rounded-[1.5rem] shadow-2xl border border-slate-100 z-50 overflow-hidden">
                  ...
                  <div className="px-5 py-3 border-b border-slate-50">
                    <div className="flex items-center gap-2 text-cyan-600">
                      <MapPin size={14} />
                      <p className="text-xs font-bold truncate">{user.city || "Pune"}, {user.state || "Maharashtra"}</p>
                    </div>
                  </div>
                  <div className="p-2">
                    <button onClick={() => { navigate("/app/profile"); setIsDropdownOpen(false); }} className="dropdown-item">
      ...
      <div className={`mobile-drawer ${isMenuOpen ? "drawer-open" : "drawer-closed"}`}>
        <div className="drawer-header">
           <span className="logo-text">BreatheSafe <span className="highlight">AI</span></span>
           <button className="close-btn" onClick={() => setIsMenuOpen(false)}><i className="fa-solid fa-xmark"></i></button>
        </div>
          ...
          <div className="p-4 mx-4 bg-slate-50 rounded-2xl mb-4">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="overflow-hidden">
                ...
                <p className="text-[10px] text-cyan-600 font-bold uppercase">{user.city || "Pune"}</p>
              </div>
            </div>
            {/* Added View Profile to Mobile Navigation */}
            <button onClick={() => { navigate("/app/profile"); setIsMenuOpen(false); }} className="w-full py-2 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-600 flex items-center justify-center gap-2">
        ...
        <nav className="drawer-nav">
          {navLinks.map((link) => (
            <NavLink key={link.path} to={link.path} onClick={() => setIsMenuOpen(false)} className="drawer-item">{link.name}</NavLink>
          ))}
          {user && <button onClick={handleLogout} className="drawer-item text-red-500">Logout</button>}
        </nav>
      </div>
