...
const { loginUser, signupUser, loading, error } = useAuthApi();
  ...
  const [phoneOtpOpen, setPhoneOtpOpen] = useState(false);
   ...
   const handleLogin = async (e) => {
    ...
    try {
      const data = await loginUser({ email: loginEmail, phone: "+91"+loginPhone, password: loginPassword });
...
const handleSignup = async (e) => {
  e.preventDefault();
  if (!signupName || !signupEmail || !signupPassword) return toast.error('Fill all fields');

  try {
    const data = await signupUser({ name: signupName, email: signupEmail, password: signupPassword });
  ...
  if (showOtpForm) {
    return (
      <div className="w-full max-w-md mx-auto bg-white rounded-xl shadow-xl p-6">
        <OtpForm email={pendingEmail} onVerified={handleOtpVerified} onBack={() => setShowOtpForm(false)} onSuccess={onSuccess} />
      </div>
    );
  }
  ...
  return (
    <>
      <motion.div className="min-h-screen flex items-center justify-evenly bg-gradient-to-br from-blue-50 via-sky-100 to-cyan-100 p-10">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        <div className="w-full max-w-md mx-auto bg-white rounded-xl shadow-xl p-6">
          ...
          <div className="text-center mb-6">
            <h1 className="text-2xl font-bold bg-gradient-to-r from-blue-600 to-green-600 bg-clip-text text-transparent">BreatheSafeAI</h1>
            <p className="text-gray-500">Your intelligent air quality companion</p>
          </div>
          ...
          <div className="flex mb-6 rounded-lg overflow-hidden bg-gray-100">
            <button
              ...
              className={`flex-1 py-2 px-4 transition-all ${activeTab === 'login' ? 'bg-white shadow-sm' : 'text-gray-600 hover:text-gray-900'}`}
            >
              Login
            </button>
            <button
              onClick={() => setActiveTab('signup')}
              className={`flex-1 py-2 px-4 transition-all ${activeTab === 'signup' ? 'bg-white shadow-sm' : 'text-gray-600 hover:text-gray-900'}`}
            >
              Sign Up
            </button>
          </div>
          ...
          <AnimatePresence mode="wait">
            {activeTab === 'login' && (
              <motion.div key="login-tab" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <GoogleButton />
      <PhoneButton
      ...
      />
                ...
                <div className="flex gap-2 mb-4 p-1 bg-gray-100 rounded-lg">
                  <button
                    ...
                    className={`flex-1 py-2 px-4 rounded-md transition-all ${loginMethod === 'email' ? 'bg-white shadow-sm' : 'text-gray-600 hover:text-gray-900'}`}
                  >
                  ...
                  <button
                    ...
                    className={`flex-1 py-2 px-4 rounded-md transition-all ${loginMethod === 'phone' ? 'bg-white shadow-sm' : 'text-gray-600 hover:text-gray-900'}`}
                  >
...
<form onSubmit={handleLogin} className="space-y-4">
  ...
  >
    {loading ? 'Logging in...' : 'Login'}
            ...
            {activeTab === 'signup' && (
              <motion.div key="signup-tab" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
  ...
  >
    {loading ? 'Creating Account...' : 'Sign Up'}
      ...
      <PhoneOtpModal open={phoneOtpOpen} onClose={() => setPhoneOtpOpen(false)} onSuccess={onSuccess} mode="login" />
    </>
  );
}
